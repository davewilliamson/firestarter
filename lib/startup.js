			...
			init(_self.config.app, function(err){
				...
				if(err){
					...
					_self.config.server.on('listening', function() {
						...
						_self.config.logger.info('+-| FireStarter Status |---------------------------------------------------------------------------------');
						_self.config.logger.info('| Application Started | Listening on port ' +  exPort + ' | Environment: ' + _self.config.app.get('env') + ' | Version: ' + _self.config.app.get('version') + ' |');
